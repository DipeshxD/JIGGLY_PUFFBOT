...
async def yardim(event):
    ...
    if tgbotusername is not None or mafia_input == "text":
        results = await event.client.inline_query(tgbotusername, "@D3VIL_BOT_SUPPORT")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of D3VILBot\n\n"
        )
        mafiacount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the D3VILBOT are [here]({url})"
